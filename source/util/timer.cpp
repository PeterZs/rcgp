...
thread_local std::vector <TimerToken::PayloadCallback> payload_callbacks;
...
void TimerToken::add_payload_callback(PayloadCallback callback)
{
	payload_callbacks.emplace_back(std::move(callback));
}

void TimerToken::add_default_report_callback()
{
	payload_callbacks.emplace_back([](const Payload &payload) {
		info("scoped timer payload report:\n%s", payload.report_string().c_str());
	});
}

void TimerToken::clear_payload_callbacks()
{
